...
//        value = "delete from user_follows u where u.id_followed = :userId and u.id_user = :userToUnfollowId"
        String request = String.format("DELETE FROM user_follows AS u WHERE u.id_followed='%s' and u.id_user='%s'", updateUnfollowUserRequest.getUserToUnfollowId(), updateUnfollowUserRequest.getUserId());
